            ...
            if(result === '[]'){
                request.get('https://api.github.com/user',{
                    ...
                    if(body.includes('Bad credentials')){
                        ...
                        let loginName = profileContent.login;
                        let query1 = {
                            'profile.login':profileContent.login
                        }
                        GithubModel.find(query1,(err,user1) =>{
                            let result1 = JSON.stringify(user1);
                            if(result1 === '[]'){
                                if(profileContent){
                                ...
                                request.get(`${starredUrl}`,{
                                    ...
                                    if(error){
                                        ...
                                        const token = jwt.sign({ login: loginName }, config.jwt.secret);
                                        return reply({ token })
                                    }
                                });
                            }else{
                ...
                request.get('https://api.github.com/user',{
                    ...
                    if(body.includes('Bad credentials')){
                        ...
                        let profileCont = JSON.parse(body);
                        const token = jwt.sign({ login: profileCont.login }, config.jwt.secret);
                        return reply({ token });
                    }
                })
            }
